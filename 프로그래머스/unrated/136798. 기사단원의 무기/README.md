...
|등급|날짜|풀이|
|---|---|------|
|Lv.1|23.05.24|(1) 1부터 number까지의 각 약수의 개수를 구한다.<p>(2) 해당 약수의 개수가 limit보다 크면 power을 반환, 그렇지 않으면 number의 약수의 개수 반환한다.<p>첫번째 접근: (1)에서 1부터 숫자까지 for문 돈다 => 시간초과<p>두번째 접근: (1)에서1부터 숫자의 제곱근까지 for문을 돌고 x2한다. (약수 제곱이 숫자일 때 제외) => 통과|
