...
|등급|날짜|풀이|
|---|---|------|
|Lv.1|23.05.24|/=2를 통하여 다음 라운드의 새 번호를 부여(홀수라면 미리 +1)<p>작은 수가 홀수, 큰 수가 짝수이면서 1 차이가 나야 같은 라운드이다(1차이나도 2, 3이라면 같은 라운드가 아닌 것처럼)<p>처음엔 for문을 log2(n)만큼만 돌게 하였으나 어차피 조건을 만족하면 return하므로 필요 x|
